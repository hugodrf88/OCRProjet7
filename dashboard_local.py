...
dict_main_variables = {'CODE_GENDER': "Sexe",  # 'Gender of the client'
                       ...
                       'NAME_HOUSING_TYPE': "Type d'habitation",
...
def preprocessor_f(df):
    ...
    # réecrivons le
    num_transformer_fill = Pipeline(
        steps=[("num_imputer", FunctionTransformer(func=fill_num, kw_args={"strategy": "mean"}))])
    cat_transformer_fill = Pipeline(
        steps=[("cat_imputer", FunctionTransformer(func=fill_cat, kw_args={"most_frequent": False}))])
...
def main():
    ...
    for k in dict_main_variables.keys():
        if k in cat_cols:
            ...
            if ("DAYS" in k) | ("AGE" in k):
                ...
                new_value = st.sidebar.date_input(dict_main_variables[k], min_value=min_date,
   ...
   # data_client_dict = data_client.to_dict(orient='records')[0]
    ...
    prediction_proba=logreg_model.predict_proba(data_client)
   ...
   # response = requests.post('http://127.0.0.3:8000/prediction', json=data_client_dict)
    st.markdown(f"<h2 style='font-size:36px;'> La prédiction est : {prediction_proba}</h2>", unsafe_allow_html=True)
    ...
    st.markdown("<h2 style='font-size:24px;'>Résultat :</h2>",
                unsafe_allow_html=True)  # st.write(f"<h1 style='text-align: center; color: grey; font-size: 55px;'>{str(rounded_prediction).strip('[]')}</h1>", unsafe_allow_html=True)

    seuil = 1 / 4
    proba = prediction_proba
    # p = proba["prediction"]
    # Définir une variable conditionnelle
    condition = proba <= seuil

    # Afficher un texte en vert si la condition est vraie, sinon en rouge
    if condition:
        st.write(
            '<div style="text-align:center; font-size: 36px; font-weight: bold; color: green;">Prêt accordé ! </div>',
            unsafe_allow_html=True)

    else:
        st.write('<div style="text-align:center; font-size: 36px; font-weight: bold; color: red;">Prêt refusé ! </div>',
                 unsafe_allow_html=True)

    st.write(
        '<div style="text-align:center; font-size: 18px; color: grey;"><i> Seules les  probabilités inférieures à {} permettent l\'obtention du prêt</i> </div>'.format(
            seuil), unsafe_allow_html=True)
    ...
    if graphiques:
        ...
        for i, supp in enumerate(supp_cols):
            ...
            # Ajouter les points pour les valeurs de succès et d'échec
            fig.add_trace(
                go.Box(x=supp_data[supp][supp_data["TARGET"] == 0], name="Succès", marker=dict(color="green")))
